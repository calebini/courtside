...
The seventh slice adds Player Management Relationships, account-requested access with administrator batch review, private Player profile updates, and validated private photo Storage. The eighth slice adds open-or-closed registration configuration, confirmed-email Account provisioning, saved language preference, and non-enumerating password recovery. The ninth slice adds the controlled, planned-by-default staging bootstrap for the first League and League Administrator assignment; its operator procedure is in `docs/operations.md`. The tenth slice adds authenticated, audited Season creation with normative standings defaults and an explicit empty playoff configuration. The eleventh slice adds batch durable-Team reconciliation, Season participation, and dependency-safe participation removal. The twelfth slice adds audited reusable Venue creation, correction, and archival while preserving existing Game references and excluding archived Venues from future scheduling. The thirteenth slice adds audited pre-freeze editing of standings point values and ranking order, preserves every unedited Season rule, and changes to a read-only summary once the first authoritative result freezes configuration. The fourteenth slice reorganizes administration around recurring work with a shared shell, a bounded state-driven League Desk, a complete Games workspace, and a separate League Setup destination without changing domain services or persistence. The fifteenth slice adds ordinary post-bootstrap League Administrator grants and revocations plus Season-Team Captain assignment, reassignment, and revocation with final-administrator protection and audit.
