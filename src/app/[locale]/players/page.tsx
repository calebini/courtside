...
export default async function PlayersPage({params, searchParams}: {params: Promise<{locale: string}>; searchParams: Promise<{result?: string}>}) {
  ...
  const [players, isAdmin] = await Promise.all([store.loadManagedPlayers(account.id), store.hasAdministrativeAccess(account.id)]);
  ...
  return <main className="dashboard-shell player-portal">
    ...
    <section className="panel access-request-panel"><p className="panel-kicker">{t('requestKicker')}</p><h2>{t('requestTitle')}</h2><p>{t('requestSummary')}</p><form action={requestPlayerAccessAction} className="stack-form compact-form"><input name="locale" type="hidden" value={locale}/><label><span>{t('playerReference')}</span><input name="playerId" required type="text"/></label><button type="submit">{t('requestAccess')}</button></form></section>
