...
export default async function PlayersPage({params, searchParams}: {params: Promise<{locale: string}>; searchParams: Promise<{result?: string}>}) {
  ...
  return <main className="dashboard-shell player-portal">
    <header className="topbar"><Link className="wordmark" href={`/${locale}`}>COURTSIDE</Link><div className="account-actions"><Link className="button-link" href={`/${locale}/stats`}>{t('memberStats')}</Link>{isAdmin ? <Link className="button-link" href={`/${locale}/admin`}>{t('leagueDesk')}</Link> : null}<span>{account.displayName}</span><form action={signOut}><input name="locale" type="hidden" value={locale}/><button className="button-link" type="submit">{t('signOut')}</button></form></div></header>
