...
Every resolution writes an Audit Record and the bracket is not internally consistent until the chosen resolution has been applied to every affected downstream participant slot.
...
10. A playoff correction conflict blocks affected automatic advancement until a League Administrator records an explicit audited resolution.
