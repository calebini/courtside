...
- A legal reschedule may change the scheduled instant while the Game remains `scheduled`, with required schedule history.
- An approved Player Management Relationship permits updates only to `display_name` and `profile_photo`. A member may hold that approved relationship to their own Player and update that Player's photo.
- After League bootstrap, existing League Administrators manage administrator assignments, but the last active League Administrator may not be revoked.
- Team Captain remains a Season-Team-scoped role marker without independent core mutation permissions in Phase 1.
- A downstream playoff correction may either halt the affected bracket path for replay or preserve the played path as an audited administrative exception.
- Roster eligibility uses the first accepted actual Game start instant, or the official forfeit-decision instant for a pre-start forfeit.
- Frozen-configuration identity and random-draw replay follow the deterministic technical recommendations in the configuration and standings sections.
