...
Path: /Users/Shared/Agent-Workspace/repos/personal/courtside/whetstone_runs/statkeeper-bounded-audit-001/audit-notes.md
Hash: 5e833d994704e51d2f27be5003a23f64fcf005e6ff25d7cc7eae98a8d723dfa6
...
Hash: 1381f56a80c1f0cbb13dd8d339ec1b7503735ab80ccd7b4ba9d8010b75489e8f
...
- relationships to other Statistical Events in the same occurrence when the vocabulary requires
  them; and
...
- relationships to another event, when required;
...
capturing | in_review | verified -> abandoned
...
- `abandoned` closes an unpublished working revision without creating or changing Player Stat
  Lines; it does not remove an earlier published revision.
...
replacement revision is transactionally accepted. Abandoning that correction does not remove or
alter the last published revision.
...
Hash: c539b44687610c70f00c1c81f3d17856340d634c6eb9a7a83542e1e1efe1c969
...
not add a separate event-to-event graph. The profile validator rejects unknown references, missing
...
`abandoned` applies only when the Game has no published revision. Abandoning an unpublished
correction returns the session to `published` and discards only that working revision.
...
`abandon_statkeeper_working_revision`:

- changes an unpublished `capturing`, `in_review`, or `verified` session with no Publication to
  `abandoned`; or
- discards an unpublished correction and returns the session to `published` without changing the
  latest Publication or member reads.
...
- published correction and abandonment transitions.
